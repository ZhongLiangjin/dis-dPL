...
class nseLoss(nn.Module):
    ...
    def nse(self, yTrue: torch.Tensor, yPred: torch.Tensor, spinUp: int = 0):
        ...
        idx = torch.tensor([i for i in range(0, yTrue.size(1)) if torch.isnan(yTrue[:, i]).sum() / yTrue.size(0) < 1/5])
...
def saveSimulation(model, loaderVal, loaderTst, outPath, config):
    ...
    with torch.no_grad():
        # for validation set
        for xVal, zVal, y in loaderVal:
            xVal, zVal, y = xVal.squeeze(0), zVal.squeeze(0), y.squeeze(0)
            output = model(xVal, zVal)
            yPred = output[:, :, 0].clone().detach().cpu().numpy()
            yTrue = y.detach().cpu().numpy()[config['data']['spinUp']:]
            # calculate metrics
            evaluateFn(true=yTrue, pred=yPred, hydroSta=config['data']['hydStations'], mode='val')

        # for test set
        for xTst, zTst, y in loaderTst:
            xTst, zTst, y = xTst.squeeze(0), zTst.squeeze(0), y.squeeze(0)
            output = model(xTst, zTst)
            bsns = ['TNH', 'JUG', 'MAQ', 'MET', 'JIM', 'HHY']
            bsnIdx = [bsns.index(bsn) for bsn in config['data']['hydStations']]
            yPred = output[:, bsnIdx, 0].clone().detach().cpu().numpy()
            yTrue = y.detach().cpu().numpy()[config['data']['spinUp']:]
            evaluateFn(true=yTrue, pred=yPred, hydroSta=config['data']['hydStations'], mode='tst')
            # save the simulated hydrological variables
            outDict['Qr'] = output.detach().cpu().numpy()[:, :len(bsns), 0]
            outDict['Ssl'] = output.detach().cpu().numpy()[:, len(bsns):, 1]
            outDict['Sss'] = output.detach().cpu().numpy()[:, len(bsns):, 2]
            outDict['E'] = output.detach().cpu().numpy()[:, len(bsns):, 3]
            outDict['Sw'] = output.detach().cpu().numpy()[:, len(bsns):, 4]

    with open(os.path.join(config['out'], outPath), 'wb') as f:
        f.write(pickle.dumps(outDict))


def evaluateFn(true, pred, hydroSta, mode):
    logging.info('*' * 100)
    logging.info(f'Calculating metrics on {mode} set')

    for i, station in enumerate(hydroSta):
        trueTemp, predTemp = true[:, i], pred[:, i]
