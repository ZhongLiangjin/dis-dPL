...
# rootDatabase = os.path.join(os.path.sep, 'Camels')  # CAMELS dataset root directory
...
testsave_path = 'checkpoints/seed_111111_seqLen_365_spinUp_365_nMul_16_staNet_35_3_256_5_365_10-5-1_7-5-3_1-1-1_3-2-1_dynSz_38_256_3_t_0131-2333'
...
for ifold in range(1, len(tarIDLst)+1):
    ...
    testmodel = loadModel(testsave_path, epoch=testepoch)
    ...
    statFile = os.path.join(testsave_path, 'statDict.json')
    ...
    # prepare the inputs
    # tsTemp = np.array([pd.Timestamp(t).day_of_year for t in TinvLst])
    # tsTrain = np.repeat(np.expand_dims(tsTemp, (0, 2)), repeats=series_norm.shape[0], axis=0)
    # zTrain = np.concatenate((series_norm, tsTrain), axis=-1)  # used as the inputs for dPL inversion gA along with attributes
    ...
    filePathLst = master.master.namePred(
          testsave_path, Ttest, 'All_Buff'+str(TestBuff), epoch=testepoch, targLst=['Qr', 'Q0', 'Q1', 'Q2', 'ET'])

    # prepare the inputs for TESTING
    testmodel.staind = TestBuff-1
    # tsTemp = np.array([pd.Timestamp(t).day_of_year for t in TtestLst])
    # tsTest = np.repeat(np.expand_dims(tsTemp, (0, 2)), repeats=seriestest_norm.shape[0], axis=0)
    # tsTest = np.concatenate([tsTrain[:, -TestBuff:, :], tsTest], axis=1)
    zTest = np.concatenate([series_norm[:, -TestBuff:, :], seriestest_norm], axis=1)
    # zTest = np.concatenate([tsTest, zTest], axis=-1)
...
outpath = os.path.join(testsave_path, f'results_ep{testepoch}')
...
for iS in range(len(keyLst)):
    statStr = keyLst[iS]
    temp = list()
    data = evaDict[0][statStr]
    data = data[~np.isnan(data)]
    dataBox.append(data)

plot.plotBoxFig(dataBox, keyLst, figsize=(6, 4), savePath=os.path.join(outpath, 'Metric_BoxPlot.png'))
