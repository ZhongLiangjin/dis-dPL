...
from model.d2PLExpHYDRO import Net
import random
import numpy as np
import logging
...
if __name__ == '__main__':
    ...
    if args.configFile is not None:
        ...
        config['data'] = {'xDataFile': 'data/data.pkl',
                          ...
                          'spinUp': [730, 730, 730],
                          'seqLen': 2190, 'winSz': 2190, 'mainIdx': 0, 'pad': 9999,
                          'hydStationBsnIdx': {'JIM': 45, 'MET': 53, 'MAQ': 35, 'JUG': 23, 'TNH': 0}}
        ...
        config['train'] = {'logLoss': False, 'wLog': 0.25, 'wStationLoss': {'TNH': 1, 'MAQ': 1, 'JIM': 1},
                           'patience': 15, 'lr': 0.005, 'clip': 3, 'epochs': 200, 'gpu': True, 'seed': 668823,
                           'outBsnIdx': outBsnIdx, 'dropout': 0.5, 'activFn': 'sigmoid'}
    ...
    config['out'] = f"./checkpoints/seed_{seed}_out_{'_'.join(hydStation)}_staTyp_{staType}_staSz_" \
    ...
    model = Net(staType=staType, staSz=staSz, dynType=dynType, dynSz=dynSz, routSz=routSz,
                ...
                activFn=config['train']['activFn'], device=device)
