...
if __name__ == '__main__':
    ...
    if args.configFile is not None:
        ...
        config['data'] = {'xDataFile': 'data/data.pkl',
                          ...
                          'spinUp': [365, 365, 365],
                          'seqLen': 365, 'winSz': 365, 'mainIdx': 0, 'pad': 9999,
        ...
        # get six random seeds using (np.random.uniform(low=0, high=1, size=5) * (10**6)).astype(int)
        # [668823, 759826, 211765, 908331, 808530, 178716]
        config['train'] = {'logLoss': False, 'wLog': 0.25, 'wStationLoss': {'TNH': 1, 'MAQ': 1, 'JIM': 1, 'HHY': 0.5},
                           'patience': 15, 'lr': 0.005, 'clip': 3, 'epochs': 200, 'gpu': True, 'seed': 178716,
                           'outBsnIdx': outBsnIdx, 'dropout': 0.5, 'activFn': 'sigmoid', 'nMul': 1, 'gpu_id': 0}
    ...
    config['out'] = f"./checkpoints/seed_{seed}_nMul_{nMul}_seqLen_{seqLen}_out_{'_'.join(hydStation)}_staTyp_{staType}_staSz_" \
