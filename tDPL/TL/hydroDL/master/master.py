...
# def readPred(out, tRange, subset, epochs=None, doMC=False, suffix=None):
#     mDict = readMasterFile(out)
...
def  loadData(optData, readX=True, readY=True):
    ...
    elif eval(optData['name']) is hydroDL.data.gages.DataframeGages:
        df = hydroDL.data.gages.DataframeGages(
            subset=optData['subset'], tRange=optData['tRange'])
        x = df.getDataTs(
            varLst=optData['varT'],
            doNorm=optData['doNorm'][0],
            rmNan=optData['rmNan'][0],
            includeLanduse=optData['includeLanduse'],
            includeWateruse=optData['includeWateruse'])
        y = df.getDataObs(
    ...
    if reTest is True:
        ...
        if closedLoop is not None:
            ...
            for iloop in range(0, nLoop):
                # model = loadModel(out, epochs=epochs)
