...
class MultiInv_HBVModel(torch.nn.Module):
    ...
    def forward(self, x, z, doDropMC=False):
        ...
        if self.pcorr is None:
            ...
            corrpara0 = Params0[:,
                        self.nhbvpm + self.nroutpm + self.nwtspm:self.nhbvpm + self.nroutpm + self.nwtspm + 1]
...
class HBVMulTDET(torch.nn.Module):
    ...
    def forward(self, x, parameters, staind, tdlst, mu, muwts, rtwts, bufftime=0, outstate=False, routOpt=False,
        ...
        Qsimmu = (torch.zeros(Pm.size(), dtype=torch.float32) + 0.001).cuda()
        ETmu = (torch.zeros(Pm.size(), dtype=torch.float32) + 0.001).cuda()
        Snowmu = (torch.zeros(Pm.size(), dtype=torch.float32) + 0.001).cuda()
        ...
        Qsimave2 = Qsimmu2.mean(-1, keepdim=True)
        ETave = ETmu.mean(-1, keepdim=True)
        Snowave = Snowmu.mean(-1, keepdim=True)
        ...
        if outstate is True:  # output states
            ...
            # return Qs
            Qall = torch.cat((Qs, Qsimave0, Qsimave1, Qsimave2, ETave, Snowave), dim=-1)
            return Qall
...
class MultiInv_HBVTDModel(torch.nn.Module):
    ...
    def __init__(self, *, ninv, nfea, nmul, hiddeninv, drinv=0.5, inittime=0, routOpt=False, comprout=False,
        ...
        # LSTM Inv + HBV Forward
        super(MultiInv_EXPHYDROTDModel, self).__init__()
        self.ninv = ninv
        self.nfea = nfea
        self.hiddeninv = hiddeninv
        self.nmul = nmul
    ...
    def forward(self, x, z, doDropMC=False):
        ...
        routpara0 = Params0[-1, :,
                    self.nhbvpm:self.nhbvpm + self.nroutpm]  # routing para dim:[Ngage, nmul*2] or [Ngage, 2]
        ...
        out = self.HBV(x, parameters=hbvpara, staind=self.staind, tdlst=self.tdlst, mu=self.nmul, muwts=wts,
                       rtwts=routpara,
                       bufftime=self.inittime, routOpt=self.routOpt, comprout=self.comprout, dydrop=self.dydrop)
        return out


class EXPHYDROMulET(torch.nn.Module):
    def __init__(self):
        super(EXPHYDROMulET, self).__init__()
    ...
    def forward(self, x, parameters, mu, muwts, rtwts, bufftime=0, outstate=False, routOpt=False, comprout=False,
        ...
        if bufftime > 0:
            with torch.no_grad():
                xinit = x[0:bufftime, :, :]
                initmodel = EXPHYDROMulET()
                Qsinit, Sw, Ssl, Sss = initmodel(xinit, parameters, mu, muwts, rtwts, bufftime=0, outstate=True,
                                                 routOpt=False, comprout=False)
        else:
            # Without warm-up bufftime=0, initialize state variables with zeros
            Ngrid = x.shape[1]
            Sw = (torch.zeros([Ngrid, mu], dtype=torch.float32) + 0.001).to(device)
            Sss = (torch.zeros([Ngrid, mu], dtype=torch.float32) + 0.001).to(device)
            Ssl = (torch.zeros([Ngrid, mu], dtype=torch.float32) + 0.001).to(device)
        ...
        ## scale the parameters
        # parTMIN, parDDF, parTMAX, parQMAX, parSMAX, parF, and parBeta
        parascaLst = [[-2, 1], [1, 8], [-1, 2], [10, 50], [100, 1500], [0, 0.1], [0, 1], [0.5, 2]]  # EXP-HYDRO para
        routscaLst = [[0, 2.9], [0, 6.5]]  # routing para

        parTMIN = parascaLst[0][0] + parameters[:, 0, :] * (parascaLst[0][1] - parascaLst[0][0])
        parDDF = parascaLst[1][0] + parameters[:, 1, :] * (parascaLst[1][1] - parascaLst[1][0])
        parTMAX = parascaLst[2][0] + parameters[:, 2, :] * (parascaLst[2][1] - parascaLst[2][0])
        parQMAX = parascaLst[3][0] + parameters[:, 3, :] * (parascaLst[3][1] - parascaLst[3][0])
        parSMAX = parascaLst[4][0] + parameters[:, 4, :] * (parascaLst[4][1] - parascaLst[4][0])
        parF = parascaLst[5][0] + parameters[:, 5, :] * (parascaLst[5][1] - parascaLst[5][0])
        parALPHA = parascaLst[6][0] + parameters[:, 6, :] * (parascaLst[6][1] - parascaLst[6][0])
        parBETA = parascaLst[7][0] + parameters[:, 7, :] * (parascaLst[7][1] - parascaLst[7][0])
        ...
        for t in range(Nstep):
            Ss = torch.clamp(Sss + Ssl, min=torch.zeros_like(parSMAX) + PRECS, max=parSMAX - PRECS)
            freezeFlag = torch.mean(Tm[t - tLen:t, :, :],
                                    dim=-1) < 0  # soil is freezing when flag is true and thawing otherwise.
            ...
            Ssl = torch.clamp(Ss - Sss, min=PRECS)

            # snow process
            PRECIP = Pm[t, :, :]
            pr = torch.mul(PRECIP, (Tm[t, :, :] >= parTMIN))
            ps = torch.mul(PRECIP, (Tm[t, :, :] < parTMIN))
            melt = torch.clamp(parDDF * (Tm[t, :, :] - parTMAX), min=torch.zeros_like(Sw), max=Sw)
            Sw = Sw - melt + ps
            ...
            Ssl = Ssl + pr + melt - ETact - Qbase
            Qsurf = torch.clamp(Ssl - (parSMAX - Sss), min=0)
            Ssl = Ssl - Qsurf
            Qsimmu[t, :, :] = Qsurf + Qbase
        ...
        if outstate is True:
            return Qs, Sw, Ssl, Sss
        else:
            return Qs  # total streamflow


class EXPHYDROMulTDET(torch.nn.Module):
    def __init__(self):
        super(EXPHYDROMulTDET, self).__init__()

    def forward(self, x, staParas, dynParas, routParas, mu, spinUp, tLen=5):
        PRECS = 3e-4  # keep the numerical calculation stable
        # Initialization to warm-up states
        device = x.device
        Ngrid = x.shape[1]
        Sw = (torch.zeros([Ngrid, mu], dtype=torch.float32) + 0.001).to(device)
        ...
        ## scale the parameters to real values from [0,1]
        # parTMIN, parDDF, parTMAX, parQMAX, parSMAX, parF, parALPHA, parBETA
        expScaLst = [[-2, 1], [1, 8], [-1, 2], [10, 50], [100, 1500], [0, 0.1], [0, 1], [0.5, 2]]
        routScaLst = [[0, 2.9], [0, 6.5]]  # routing para

        allParas = torch.cat((staParas, dynParas), dim=-2)
        allParasTrans = torch.zeros_like(allParas)
        for ip in range(len(expScaLst)):
            allParasTrans[:, :, ip, :] = expScaLst[ip][0] + allParas[:, :, ip, :] * (expScaLst[ip][1] - expScaLst[ip][0])
        Nstep, Ngrid = P.size()
        ...
        Snowmu = (torch.zeros(Pm.size(), dtype=torch.float32) + 0.001).to(device)
        Sssmu = (torch.zeros(Pm.size(), dtype=torch.float32) + 0.001).to(device)
        Sslmu = (torch.zeros(Pm.size(), dtype=torch.float32) + 0.001).to(device)
        ...
        for t in range(Nstep):
            paraLst = []
            for ip in range(len(expScaLst)):  # unpack EXP-HYDRO parameters
                paraLst.append(allParasTrans[t, :, ip, :])

            parTMIN, parDDF, parTMAX, parQMAX, parSMAX, parF, parALPHA, parBETA = paraLst
            # determine frozen water and unfrozen water in soil
            Ss = torch.clamp(Sss + Ssl, min=torch.zeros_like(parSMAX) + PRECS, max=parSMAX - PRECS)
            antTm = Tm[t-tLen+1:t+1, :, :] if t - tLen >= 0 else Tm[:t+1, :, :]
            freezeFlag = torch.mean(antTm, dim=0) < 0  # soil is freezing when flag is true and thawing otherwise.
            minSss = torch.where(freezeFlag, Sss, torch.zeros_like(Sss))
            maxSss = torch.where(freezeFlag, Ss, Sss)
            Sss = minSss + (maxSss - minSss) * parALPHA
            Ssl = torch.clamp(Ss - Sss, min=PRECS)
        ...
        ETave = ETmu.mean(-1, keepdim=True)
        Snowave = Snowmu.mean(-1, keepdim=True)
        Sssave = Sssmu.mean(-1, keepdim=True)
        Sslave = Sslmu.mean(-1, keepdim=True)

        # get the initial average
        Qsim = Qsimmu.mean(-1)

        # scale two routing parameters
        tempa = routScaLst[0][0] + routParas[:, 0] * (routScaLst[0][1] - routScaLst[0][0])
        tempb = routScaLst[1][0] + routParas[:, 1] * (routScaLst[1][1] - routScaLst[1][0])
        routa = tempa.repeat(Nstep, 1).unsqueeze(-1)
        routb = tempb.repeat(Nstep, 1).unsqueeze(-1)
        UH = UH_gamma(routa, routb, lenF=15)  # lenF: folter
        rf = torch.unsqueeze(Qsim, -1).permute([1, 2, 0])  # dim:gage*var*time
        ...
        # return Qs
        Qall = torch.cat((Qs[spinUp:], Sslave[spinUp:], Sssave[spinUp:], ETave[spinUp:], Snowave[spinUp:]), dim=-1)
        return Qall


class dPL(torch.nn.Module):
    def __init__(self, nAttr, nMet, hidFC, outFC, spinUp, nKernel, kernelSz, stride, poolSz,
                 dynInLSTM, dynHidLSTM, dynOutLSTM, nmul: int = 16,  dropout: float = 0.5,
                 device: Union[str, torch.device] = 'cpu'):
        super(dPL, self).__init__()
        self.staNet = StaParasConv1DMLP(nx=nAttr, nz=nMet, ny=outFC*nmul+2, inLen=spinUp, hidden=hidFC, nKernel=nKernel,
    ...
    def forward(self, x):
        staPara = torch.sigmoid(self.staNet(x))  # (ngauge, npara)
        dynPara = torch.sigmoid(self.dynNet(x))   # (L, ngague, npara)
        return (staPara, dynPara)
...
class StaParasConv1DMLP(torch.nn.Module):
    ...
    def forward(self, x):
        forcing, attrs = x[:self.inLen, :, :3], x[:, :, 3:]
        encoded = self.convLayers(forcing.permute(1, 2, 0))
...
class MultiInv_EXPHYDROTDModel(torch.nn.Module):
    # class for dPL + EXP-HYDRO with multiple components and some dynamic parameters
    def __init__(self, staSz, dynSz, nmul, device='cpu', dropout=0.5):
        # LSTM Inv + HBV Forward
        super(MultiInv_EXPHYDROTDModel, self).__init__()
        nAttr, nMet, hidFC, outFC, spinUp, nKernel, kernelSz, stride, poolSz = staSz
        dynInLSTM, dynHidLSTM, dynOutLSTM = dynSz

        self.nmul = nmul
        self.nStaPara = outFC
        self.nDynPara = dynOutLSTM
        self.EXPHYDRO = EXPHYDROMulTDET()
        self.spinUp = spinUp

        # get the total number of parameters
        self.dPL = dPL(nAttr, nMet, hidFC, outFC, spinUp, nKernel, kernelSz, stride, poolSz, dynInLSTM, dynHidLSTM,
                       dynOutLSTM, nmul=nmul, dropout=dropout, device=device)

    def forward(self, x, z, doDropMC=False):
        staPara, dynPara = self.dPL(z)
        ntstep = dynPara.shape[0]
        ngage = dynPara.shape[1]
        # print(Params0)
        routPara = staPara[:, -2:]
        expStaPara = staPara[:, :-2].unsqueeze(0).repeat(ntstep, 1, 1).view(ntstep, ngage, self.nStaPara, self.nmul)
        expDynPara = dynPara.view(ntstep, ngage, self.nDynPara, self.nmul)
        out = self.EXPHYDRO(x, expStaPara, expDynPara, routPara, self.nmul, self.spinUp)
        return out
