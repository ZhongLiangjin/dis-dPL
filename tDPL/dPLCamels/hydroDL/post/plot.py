...
def plotBoxFig(data, label=None, figsize=(6, 4), savePath=None):
    plt.rcParams['font.family'] = 'Arial'
    sns.set(style='ticks')
    colors = sns.color_palette('pastel', n_colors=2)
    fig, ax = plt.subplots(ncols=1, nrows=1, figsize=figsize)
    bp = ax.boxplot(data, patch_artist=True, notch=True, showfliers=False, widths=0.15, showmeans=True,
    ...
    for kk in range(0, len(bp['boxes'])):
        plt.setp(bp['boxes'][kk], facecolor=colors[kk])
    ax.set_xticks([])
    yMax = np.array([temp.max() for temp in data]).max()
